...
def getNormal(vec0,vec1):
    perp = [vec0[1] * vec1[2] - vec0[2] * vec1[1],
            vec0[2] * vec1[0] - vec0[0] * vec1[2],
            vec0[0] * vec1[1] - vec0[1] * vec1[0]]
    return perp

def draw_polygons( polygons, screen, color ):
    for i in range(int(len(polygons)/3)):
        currind = i * 3
        p0 = polygons[currind]
        p1 = polygons[currind + 1]
        p2 = polygons[currind + 2]
        #Cull check
        vec0 = [p2[0] - p1[0], p2[1] - p1[1], p2[2] - p1[2]]
        vec1 = [p0[0] - p1[0], p0[1] - p1[1], p0[2] - p1[2]]
        perp = getNormal(vec0,vec1)
        #mag = math.pow(math.pow(perp[0],2) + math.pow(perp[1],2) + math.pow(perp[2],2), .5)
        #perp[2] /= mag #Normalize z component
        dotprod = perp[2] #z axis [0,0,1] with perp
        if dotprod < 0: #Do not draw if facing away
            continue
        for i in range(2):
            p0[i] = int(p0[i])
            p1[i] = int(p1[i])
